...
class BoldRetryProcessor
{
    ...
    private function processWebhookData($data)
    {
        ...
        // CORRECCIÓN CRÍTICA: Extraer order_id desde metadata.reference si no existe directo
        $orderId = '';
        ...
        if (empty($orderId) || empty($status)) {
            throw new Exception('Datos de pago incompletos: order_id o status faltante');
        }
    ...
    {
        // Extraer datos del cliente - múltiples fuentes posibles
        $customerEmail = '';
        $customerName = 'Cliente Bold PSE';
        $customerPhone = '';
        ...
        if (isset($paymentData['customer'])) {
            ...
            $customerEmail = $paymentData['payer_email'];
        }

        // Para tarjetas, intentar obtener el nombre del portador
        if (isset($paymentData['card']['cardholder_name']) && !empty($paymentData['card']['cardholder_name'])) {
            $customerName = $paymentData['card']['cardholder_name'];
        }

        // Sanitizar datos
        $customerEmail = htmlspecialchars(trim($customerEmail), ENT_QUOTES, 'UTF-8');
        $customerName = htmlspecialchars(trim($customerName), ENT_QUOTES, 'UTF-8');
        $customerPhone = htmlspecialchars(trim($customerPhone), ENT_QUOTES, 'UTF-8');

        // Crear descripción del pedido
        $pedidoTexto = "Pedido Bold PSE - " . $orderId . " - Monto: $" . number_format($amount, 0, ',', '.');
        ...
        $stmt = $this->conn->prepare("
            INSERT INTO pedidos_detal (
                ...
                fecha,
                estado
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'PSE Bold', ?, 'pagado', ?, NOW(), 'sin_enviar')
        ");

        $direccion = "Dirección por confirmar";
        $personaRecibe = $customerName;
        $horarios = "Horario por confirmar";
