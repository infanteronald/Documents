...
try {
    // Incluir conexión desde el directorio correcto con múltiples intentos
    $possible_paths = [
    ...
    if (!$conexion_path) {
        throw new Exception('Archivo de conexión no encontrado en ninguna ubicación esperada');
    }
    
    // Log de debugging
    error_log("Bold Status API - Usando conexión: " . $conexion_path);
    
    require_once $conexion_path;
    
    if (!isset($conn) || !$conn) {
        throw new Exception('Variable de conexión $conn no disponible después de incluir ' . $conexion_path);
    }
