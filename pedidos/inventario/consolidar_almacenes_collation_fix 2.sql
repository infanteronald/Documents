-- ========================================
-- SCRIPT DE CONSOLIDACIÓN - CORRECCIÓN DE COLLATION
-- Soluciona problemas de collation entre tablas
-- ========================================
...
-- PASO 2: CREAR TABLA ALMACENES CONSOLIDADA CON COLLATION UNIFORME
CREATE TABLE IF NOT EXISTS almacenes_consolidado (
    id INT AUTO_INCREMENT PRIMARY KEY,
    codigo VARCHAR(20) NOT NULL UNIQUE COLLATE utf8mb4_unicode_ci,
    nombre VARCHAR(100) NOT NULL UNIQUE COLLATE utf8mb4_unicode_ci,
    descripcion TEXT DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    direccion TEXT DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    ubicacion VARCHAR(255) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    telefono VARCHAR(20) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    capacidad_maxima INT DEFAULT 0,
    encargado VARCHAR(100) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    icono VARCHAR(10) DEFAULT '🏪' COLLATE utf8mb4_unicode_ci,
...
-- PASO 4: AGREGAR ALMACENES DESDE PRODUCTOS (CON COLLATION EXPLÍCITA)
...
WHERE p.almacen IS NOT NULL 
  AND p.almacen != ''
  AND p.almacen NOT IN (
    SELECT ac.nombre FROM almacenes_consolidado ac
  );
...
CREATE TABLE IF NOT EXISTS inventario_almacen_new (
    ...
    ubicacion_fisica VARCHAR(100) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
...
-- Usar subconsulta para evitar problemas de collation
INSERT IGNORE INTO inventario_almacen_new (producto_id, almacen_id, stock_actual, stock_minimo, stock_maximo)
SELECT 
    p.id as producto_id,
    (SELECT ac.id FROM almacenes_consolidado ac WHERE ac.nombre = p.almacen LIMIT 1) as almacen_id,
...
WHERE p.almacen IS NOT NULL 
  AND p.almacen != ''
  AND EXISTS (
    SELECT 1 FROM almacenes_consolidado ac WHERE ac.nombre = p.almacen
  );
...
CREATE TABLE IF NOT EXISTS movimientos_inventario_new (
    ...
    motivo VARCHAR(255) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    documento_referencia VARCHAR(100) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    usuario_responsable VARCHAR(100) DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    almacen_destino_id INT NULL,
    fecha_movimiento TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    observaciones TEXT DEFAULT NULL COLLATE utf8mb4_unicode_ci,
    FOREIGN KEY (producto_id) REFERENCES productos(id) ON DELETE CASCADE,
    FOREIGN KEY (almacen_id) REFERENCES almacenes_consolidado(id),
...
-- PASO 13: VERIFICAR COLLATIONS
SELECT 
    'VERIFICACIÓN COLLATIONS' as info,
    'productos.almacen' as tabla_campo,
    (SELECT COLLATION_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = 'productos' AND COLUMN_NAME = 'almacen' AND TABLE_SCHEMA = DATABASE()) as collation_actual;

SELECT 
    'VERIFICACIÓN COLLATIONS' as info,
    'almacenes_consolidado.nombre' as tabla_campo,
    (SELECT COLLATION_NAME FROM information_schema.COLUMNS WHERE TABLE_NAME = 'almacenes_consolidado' AND COLUMN_NAME = 'nombre' AND TABLE_SCHEMA = DATABASE()) as collation_actual;

-- MENSAJE FINAL
SELECT 'MIGRACIÓN COMPLETADA CON CORRECCIÓN DE COLLATION' as mensaje;
