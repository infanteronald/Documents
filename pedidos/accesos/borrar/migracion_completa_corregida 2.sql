...
-- VERSIÓN CORREGIDA PARA MARIADB
-- ============================================
...
-- ============================================
-- PARTE 4: AGREGAR LLAVES FORÁNEAS (MÉTODO COMPATIBLE)
-- ============================================

-- Verificar si las llaves foráneas ya existen antes de crearlas
...
CALL add_foreign_key_if_not_exists('usuarios', 'fk_usuarios_creado_por', 'FOREIGN KEY (creado_por) REFERENCES usuarios(id) ON DELETE SET NULL');
CALL add_foreign_key_if_not_exists('usuarios', 'fk_usuarios_modificado_por', 'FOREIGN KEY (modificado_por) REFERENCES usuarios(id) ON DELETE SET NULL');

-- Agregar llaves foráneas para roles
CALL add_foreign_key_if_not_exists('roles', 'fk_roles_creado_por', 'FOREIGN KEY (creado_por) REFERENCES usuarios(id) ON DELETE SET NULL');
CALL add_foreign_key_if_not_exists('roles', 'fk_roles_modificado_por', 'FOREIGN KEY (modificado_por) REFERENCES usuarios(id) ON DELETE SET NULL');

-- Agregar llave foránea para pedidos_detal
CALL add_foreign_key_if_not_exists('pedidos_detal', 'fk_pedidos_usuario', 'FOREIGN KEY (usuario_id) REFERENCES usuarios(id) ON DELETE SET NULL');

-- Limpiar procedimiento temporal
DROP PROCEDURE add_foreign_key_if_not_exists;
