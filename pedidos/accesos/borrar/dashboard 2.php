...
$user_roles = $session_info['acc_roles'];
...
$stats_query = "SELECT 
    (SELECT COUNT(*) FROM acc_usuarios WHERE activo = 1) as total_usuarios,
    (SELECT COUNT(*) FROM acc_roles WHERE activo = 1) as total_roles,
    (SELECT COUNT(*) FROM acc_modulos WHERE activo = 1) as total_modulos,
    (SELECT COUNT(*) FROM acc_sesiones WHERE activa = 1 AND fecha_expiracion > NOW()) as sesiones_activas,
    (SELECT COUNT(*) FROM acc_auditoria_accesos WHERE DATE(fecha_accion) = CURDATE()) as acciones_hoy";
    ...
    <div class="container">
        ...
        <div class="quick-access-section" style="background: var(--bg-secondary); border: 1px solid var(--border-color); border-radius: var(--border-radius); padding: var(--space-lg); margin-bottom: var(--space-lg);">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: var(--space-md);">
                ...
                <?php if ($auth->hasAnyPermission('acc_usuarios', ['leer'])): ?>
