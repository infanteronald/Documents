/**
 * Corregir vista para soportar ambos nombres: usuarios y acc_usuarios
 * Problema: Los archivos PHP buscan 'usuarios' pero la vista solo tiene 'acc_usuarios'
 */

-- Eliminar vista actual
DROP VIEW IF EXISTS acc_vista_permisos_usuario;

-- Recrear vista con UNION para soportar ambos nombres de módulo usuarios
...
-- Agregar entrada adicional para 'usuarios' sin prefijo (para compatibilidad con archivos PHP existentes)
SELECT DISTINCT
    u.id as usuario_id,
    u.nombre as usuario_nombre,
    u.email as usuario_email,
    r.id as rol_id,
    r.nombre as rol_nombre,
    'usuarios' as modulo,  -- Sin prefijo para compatibilidad
    p.tipo_permiso,
    p.descripcion as permiso_descripcion
FROM acc_usuarios u
INNER JOIN acc_usuario_roles ur ON u.id = ur.usuario_id AND ur.activo = 1
