        ...
        try {
            ...
            if ($user) {
                ...
                $query = "INSERT INTO auditoria_accesos (usuario_id, accion, modulo, descripcion, ip_address, user_agent) 
                          VALUES (?, 'password_reset_request', 'usuarios', 'Solicitud de recuperación de contraseña', ?, ?)";
                $stmt = $conn->prepare($query);
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        try {
            ...
            if ($result->num_rows === 1) {
                ...
                $user_agent = $_SERVER['HTTP_USER_AGENT'] ?? '';
                $query = "INSERT INTO auditoria_accesos (usuario_id, accion, modulo, descripcion, ip_address, user_agent) 
                          VALUES (?, 'password_reset_completed', 'usuarios', 'Contraseña restablecida exitosamente', ?, ?)";
                $stmt = $conn->prepare($query);
