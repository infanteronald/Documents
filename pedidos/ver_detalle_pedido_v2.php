<?php
/**
 * Ver Detalle de Pedido - Versión Final Unificada
 * Sequoia Speed - Diseño moderno VSCode Dark + Apple fonts
 */
...
if ($id && $id > 0) {
    try {
        // Obtener datos del pedido usando método compatible
        $stmt = $conn->prepare("SELECT id, nombre_cliente, email_cliente, telefono_cliente, estado, fecha_pedido, metodo_pago, direccion_entrega, notas FROM pedidos_detal WHERE id = ? LIMIT 1");
        $stmt->bind_param("i", $id);
        $stmt->execute();

        // Usar bind_result para compatibilidad
        $stmt->bind_result($p_id, $p_nombre_cliente, $p_email_cliente, $p_telefono_cliente, $p_estado, $p_fecha_pedido, $p_metodo_pago, $p_direccion_entrega, $p_notas);

        if ($stmt->fetch()) {
            $pedido_encontrado = true;
            // Crear array asociativo manualmente
            $p = array(
                'id' => $p_id,
            ...
            // Obtener productos del pedido usando método compatible
            ...
            $stmt_detalle->bind_result($prod_nombre, $prod_precio, $prod_cantidad, $prod_talla);
            ...
            while ($stmt_detalle->fetch()) {
                ...
                $total_productos += floatval($prod_precio) * intval($prod_cantidad);
            }
            $stmt_detalle->close();
        } else {
